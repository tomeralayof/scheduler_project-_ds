...
void ClearList() /*create,peek,deque,clear*/
{
	...
	assert(0 == PQEnqueue(my_list, &x1));

	assert(0 == PQEnqueue(my_list, &x2));
	assert(0 == PQEnqueue(my_list, &x3));
	assert(0 == PQEnqueue(my_list, &x4));
	...
	assert(NULL == PQErase(my_list,DoesIntMatch,&not_exist));
